...
function fb_share(user_type){
    FB.ui({
        ...
        navigate_to_page(user_type, page_type);
    });
}
            ...
            || from_state == '')
        {
            ...
            $.ajax({
                ...
                success:function(response) {
                    var page_type = $("input[name='page_type']").val();
                     if (response.social_auth_ses && response.inv_chk_login && response.exist_user != "new_user"){
                        get_advice();
                    }
                    if (response.exist_user == "user_exist") {
                        $('#check_social_media_details').modal('hide');
                        $('#upwrdz_soc_perm_modal').modal('hide');
                        // Commented temporaliy
                        navigate_to_page(user_type, page_type);
                    } else if (response.exist_user == "new_user") {
                        $('#check_social_media_details').modal('hide');
                        $('#upwrdz_soc_perm_modal').modal('hide');
                        $('[name="share_button"]')
                            .attr('onclick', "fb_share("+"'"+user_type+"'"+")");
                        // Commented temporaliy
                        $('[name="cancel_share"]')
                        .attr('onclick', "navigate_to_page("+"'"+user_type+"'"+","+"'"+page_type+"'"+")");
            ...
            check_social_email(response.email);
        }
    });
}
...
function navigate_to_page(user_type=undefined, page_type=undefined){
    if (user_type == 'member') {
