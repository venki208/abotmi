...
class ClickMeetingRestClient:
    ...
    def sendRequest(self, method, path, params = None, format_response = True,
                                                                is_upload_file = False):
        '''
        Sending API request to get/post to ClickMeeting
        '''
    ...
    def conferences(self, status = 'active', page = 1):
        '''
        API for getting active state meeting conferences by page
        '''
        return self.sendRequest(
            'GET', 'conferences/'+'%s' % status + '?page=' + '%d' % page)

    def conference(self, room_id):
        '''
        Getting all meetings under room
        '''
        return self.sendRequest('GET', 'conferences/'+'%s' % room_id)

    def addConference(self, params):
        '''
        Adding conference to ClickMeeting
        '''
        return self.sendRequest('POST', 'conferences', params)

    def editConference(self, room_id, params):
        '''
        Editing the conference in ClickMeeting room
        '''
    ...
    def conferenceAutologinHash(self, room_id, params):
        '''
        API for posting auto Login Hash key
        '''
    ...
    def addConferenceRegistration(self, room_id, params):
        return self.sendRequest(
            'POST', 'conferences/'+'%s' % room_id+'/registration', params)

    def conferenceRegistrations(self, room_id, status):
        return self.sendRequest(
            'GET', 'conferences/'+'%s' % room_id+'/registrations/'+status)
    ...
    def fileLibraryContent(self, file_id):
        return self.sendRequest(
            'GET',
            'file-library/'+'%s' % file_id+'/download', 
            None, 
            False
        )
    ...
    def deleteConferenceRecording(self, room_id, recording_id):
        return self.sendRequest(
            'DELETE', 'conferences/'+'%s' % room_id+'/recordings/'+'%s' % recording_id)
    ...
    def sendConferenceEmailInvitations(self, room_id, lang='en', params=None):
        return self.sendRequest(
            'POST', 'conferences/'+'%s' % room_id+'/invitation/email/'+lang, params)
